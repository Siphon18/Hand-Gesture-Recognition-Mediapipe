...
def draw_transparent_box(img, pt1, pt2, alpha=0.5, beta=0.5):
    ...
    # If sub-image is valid (not empty)
    if sub_img.size > 0:
        white_rect = np.ones(sub_img.shape, dtype=np.uint8) * 255
        res = cv2.addWeighted(sub_img, alpha, white_rect, beta, 1.0)
        
        # If the result is not None, assign it back to the image
        if res is not None:
            img[y1:y2, x1:x2] = res
        else:
            print("Error: cv2.addWeighted returned None")
    else:
        print(f"Error: Invalid subregion with coordinates {(x1, y1)} to {(x2, y2)}")



# Assuming landmarks is a NormalizedLandmarkList (i.e., a list of landmarks)
def draw_fingertips(landmarks, finger_states, img):
    for i in range(5):  # Loop over fingers
        # Access the landmark coordinates using the landmark attribute
        fingertip = landmarks.landmark[4 * (i + 1)]  # Get fingertip (landmark 4*i+4)
        
        # Continue drawing and processing the fingertip
        x, y = int(fingertip.x * img.shape[1]), int(fingertip.y * img.shape[0])
        cv2.circle(img, (x, y), 5, (0, 255, 0), -1)  # Draw fingertip on image
...
def draw_bounding_box(landmarks, detected_gesture, img):
    ...
    w = img.shape[1]
    h = img.shape[0]
    tor = int(w / 40)  # Padding around the bounding box
    ...
    # Ensure the bounding box does not exceed image bounds
    x_min = max(0, x_min - tor)
    y_min = max(0, y_min - tor)
    x_max = min(w, x_max + tor)
    y_max = min(h, y_max + tor)

    # Draw transparent box as background for the label
    draw_transparent_box(img, (x_min, y_min - 40), (x_max, y_min))

    # Draw the rectangle bounding box
    cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (255, 0, 0), 2, lineType=cv2.LINE_AA)

    # Draw the gesture label on the image
    cv2.putText(img, f'{detected_gesture}', (x_min + 5, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
