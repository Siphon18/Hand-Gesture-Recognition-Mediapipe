...
class GestureDetector:
    ...
    def detect_gesture(self, img, mode, draw=True):
        hands = self.hand_detector.detect_hands(img)  # Detect hands using HandDetector
    ...
    # Check if self.hand_detector.hands is not empty
     if self.hand_detector.hands:
        # If self.hand_detector.hands is a list or iterable, iterate through it
        if isinstance(self.hand_detector.hands, list):
        ...
        # If self.hand_detector.hands is an object and not iterable, handle it accordingly
        elif hasattr(self.hand_detector.hands, 'landmarks'):
            # Assuming self.hand_detector.hands is a single hand object or a container
            hand = self.hand_detector.hands  # Use it directly if it's a single hand object
            self.hand_detector.draw_landmarks(img)
            draw_fingertips(hand['landmarks'], self.finger_states, img)

    def draw_gesture_box(self, img):
        if self.hand_detector.decoded_hands:
